...
func UpdateNodePoolKubernetesVersionOrImageType(
	...
	if npVersion != "" && utils.StringValue(upstreamNodePool.Version) != npVersion {
		logrus.Infof("updating kubernetes versionon of node pool [%s] from %s to %s on cluster [%s (id: %s)]", *nodePool.Name, npVersion, *upstreamNodePool.Version, config.Spec.ClusterName, config.Name)
