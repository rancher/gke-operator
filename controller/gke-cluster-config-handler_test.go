...
var _ = Describe("createCluster", func() {
	...
	It("should not create a cluster if the service account email address is not valid", func() {
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("field [serviceAccount] must either be an empty string, 'default' or set to a valid email address for nodepool [test-node-pool] in non-nil cluster [test-cluster (test-cluster)]"))
	})
})
